			...
			<?php if (isset($data['dirs'])): ?>
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<?php foreach ($data['dirs'] as $dir): ?><?php if ($dir==$data['dir']): ?><?php echo $dir ?>
							<br/>
						<?php else: ?>
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<?php foreach ($data['files'] as $datekey=>$datevalue): ?>
						<strong><?php echo date($date, mktime(12, 0, 0, substr($datekey, 4, 2), substr($datekey, 6, 2), substr($datekey, 0, 4))) ?>:</strong>
						<br/>
						<?php foreach ($datevalue as $file): ?>

							<?php if ($file==$data['file']): ?><?php if (!@checkdate(substr($file, 4, 2), substr($file, 6, 2), substr($file, 0, 4))): ?><?php echo substr($file, 0) ?>
								<br/>
							<?php else: ?><?php echo substr($file, 9) ?><br/>
							<?php endif ?><?php else: ?><?php if (!@checkdate(substr($file, 4, 2), substr($file, 6, 2), substr($file, 0, 4))): ?>
								<a href="?dir=<?php echo $data['dir'] ?>&amp;file=<?php echo $file ?>"><?php echo substr($file, 0) ?></a>
								<br/>
							<?php else: ?>
								<a href="?dir=<?php echo $data['dir'] ?>&amp;file=<?php echo $file ?>"><?php echo substr($file, 9) ?></a>
								<br/>
