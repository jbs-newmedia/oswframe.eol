 ...
 *
 */
class osW_Tool_ProjectManager extends osW_Tool_Object {
	...
	public function checkPackageList($packagelist) {
		...
		foreach ($packagelist as $key=>$package) {
			...
			if (!isset($package['info']['group'])||(!in_array($package['info']['group'], array('frame','example','project')))) {
	...
	/**
	 *
	 * @return osW_Tool_ProjectManager
	 */
