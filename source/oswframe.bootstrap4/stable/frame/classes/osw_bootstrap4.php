 ...
 *
 */
class osW_Bootstrap4 extends osW_Object {
	...
	public function load($version='current') {
		...
		if ($this->issetcore!==true) {
			...
			if (osW_Cache::getInstance()->exists(__CLASS__, $file, '.js')!==true) {
				$cache_content=file_get_contents(vOut('settings_abspath').'frame/resources/bootstrap4/'.$version.'/js/bootstrap.bundle.js');
				#$cache_content=str_replace('url(\'../fonts/', 'url(\'__DIR__/frame/resources/bootstrap4/'.$version.'/fonts/', $cache_content);
			...
			if (osW_Cache::getInstance()->exists(__CLASS__, $file, '.css')!==true) {
				$cache_content=file_get_contents(vOut('settings_abspath').'frame/resources/bootstrap4/'.$version.'/css/bootstrap.css');
				#$cache_content=str_replace('url(\'../fonts/', 'url(\'__DIR__/frame/resources/bootstrap4/'.$version.'/fonts/', $cache_content);
	...
	public function loadPlugin($plugin_name, $plugin_option=[]) {
		...
		if (file_exists(vOut('settings_abspath').'modules/'.vOut('frame_current_module').'/autoloader/bootstrap4.inc.php')) {
			include(vOut('settings_abspath').'modules/'.vOut('frame_current_module').'/autoloader/bootstrap4.inc.php');
		} elseif (file_exists(vOut('settings_abspath').'modules/'.vOut('project_default_module').'/autoloader/bootstrap4.inc.php')) {
			include(vOut('settings_abspath').'modules/'.vOut('project_default_module').'/autoloader/bootstrap4.inc.php');
		} else {
			include(vOut('settings_abspath').'frame/autoloader/bootstrap4.inc.php');
		}
	}
