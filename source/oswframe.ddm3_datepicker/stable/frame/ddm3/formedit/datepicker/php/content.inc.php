...
if (osW_Settings::getInstance()->getAction()=='doedit') {
	...
	$date_format=$this->getEditElementOption($ddm_group, $element, 'date_format');
	...
	$this->setDoEditElementStorage($ddm_group, $element, $date_new);
}
...
osW_Template::getInstance()->addJSCodeHead('
	$(function(){
		$("#'.$element.'").datepicker({
			autoSize: true,
			firstDay: '.$this->getEditElementOption($ddm_group, $element, 'day_first').',
			changeYear: '.$this->getEditElementOption($ddm_group, $element, 'year_change').',
			nextText: "&rarr;",
			prevText: "&larr;",
			yearRange: "'.$this->getEditElementOption($ddm_group, $element, 'year_min').':'.$this->getEditElementOption($ddm_group, $element, 'year_max').'",
			...
			showOn: "button",
			buttonText: "'.$this->getEditElementOption($ddm_group, $element, 'text_button').'",
		});
	});
');
