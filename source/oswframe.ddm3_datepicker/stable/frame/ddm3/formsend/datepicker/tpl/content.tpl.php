...
?>
	...
	<tr class="table_ddm_row table_ddm_row_data <?php echo osW_Template::getInstance()->getColorClass('table_ddm_rows', ['table_ddm_row_cella', 'table_ddm_row_cellb']) ?> <?php if (osW_Form::getInstance()->getFormError($element)===true): ?>table_ddm_row_error<?php endif ?> ddm_element_<?php echo $this->getSendElementValue($ddm_group, $element, 'id') ?> ">
		...
		<td class="table_ddm_col table_ddm_col_data table_ddm_col_form">
			<?php if (strlen($this->getSendElementStorage($ddm_group, $element))==8): ?><?php $date_format=$this->getSendElementOption($ddm_group, $element, 'date_format') ?><?php $date_format=str_replace('%', '%osw_tmp_ddm3_%', $date_format) ?><?php $date_format=str_replace('%osw_tmp_ddm3_%d', substr($this->getSendElementStorage($ddm_group, $element), 6, 2), $date_format) ?><?php $date_format=str_replace('%osw_tmp_ddm3_%m', substr($this->getSendElementStorage($ddm_group, $element), 4, 2), $date_format) ?><?php $date_format=str_replace('%osw_tmp_ddm3_%Y', substr($this->getSendElementStorage($ddm_group, $element), 0, 4), $date_format) ?><?php else: ?><?php $date_format=''; ?><?php endif ?>

			<?php if ($this->getSendElementOption($ddm_group, $element, 'readonly')===true): ?><?php echo h()->_outputString($date_format) ?><?php else: ?><?php echo osW_Form::getInstance()->drawTextField($element, $date_format, ['input_class'=>'table_ddm_col_form_date']) ?><?php endif ?>
		</td>
	</tr>
