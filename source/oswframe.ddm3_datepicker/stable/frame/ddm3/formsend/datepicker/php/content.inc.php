...
if (osW_Settings::getInstance()->getAction()=='dosend') {
	...
	$date_format=$this->getSendElementOption($ddm_group, $element, 'date_format');
	...
	$this->setDoSendElementStorage($ddm_group, $element, $date_new);
}
...
osW_Template::getInstance()->addJSCodeHead('
	$(function(){
		$("#'.$element.'").datepicker({
			autoSize: true,
			firstDay: '.$this->getSendElementOption($ddm_group, $element, 'day_first').',
			changeYear: '.$this->getSendElementOption($ddm_group, $element, 'year_change').',
			nextText: "&rarr;",
			prevText: "&larr;",
			yearRange: "'.$this->getSendElementOption($ddm_group, $element, 'year_min').':'.$this->getSendElementOption($ddm_group, $element, 'year_max').'",
			...
			showOn: "button",
			buttonText: "'.$this->getSendElementOption($ddm_group, $element, 'text_button').'",
		});
	});
');
