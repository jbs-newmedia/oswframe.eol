...
define('OSW_TOKEN_NAME', 'moxiemanager_token');
define('OSW_CHECK_SESSION', false);
$root_dir=realpath(__DIR__.'/../../../');
include($root_dir.'/frame/includes/sid_trans.php');

$token='';
if (isset($_SESSION[OSW_TOKEN_NAME])) {
	$token=$_SESSION[OSW_TOKEN_NAME];
} else {
	die('Token-Error');
}

if ($token!=md5(getFrameConfig('settings_protection_salt').serialize($_SESSION['moxiemanager_options_'.$token]).session_id())) {
	die('Session-Error');
}
...
$moxieManagerConfig['SessionAuthenticator.logged_in_key'] = 'moxiemanager_login_'.$token;
$moxieManagerConfig['SessionAuthenticator.user_key'] = 'moxiemanager_user_'.$token;
$moxieManagerConfig['SessionAuthenticator.config_prefix'] = 'moxiemanager';

foreach ($_SESSION['moxiemanager_options_'.$token] as $key => $value) {
	if (is_array($value)) {
		$moxieManagerConfig[$key]=implode(',', $value);
	} elseif (is_bool($value)) {
		if ($value===true) {
			$moxieManagerConfig[$key]=true;
		} else {
			$moxieManagerConfig[$key]=false;
		}
	} else {
		$moxieManagerConfig[$key]=$value;
	}
}

exec('cd '.$moxieManagerConfig['filesystem.rootpath'].'; find -type f -print0 | xargs -0 chmod '.intval(getFrameConfig('settings_chmod_file', 'integer')).'; find -type d -print0 | xargs -0 chmod '.intval(getFrameConfig('settings_chmod_dir', 'integer')).';');
