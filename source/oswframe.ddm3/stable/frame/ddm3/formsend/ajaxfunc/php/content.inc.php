...
foreach (osW_DDM3::getInstance()->getSendElements($ddm_group) as $element=>$options) {
...
foreach ($ajax['logic'] as $group=>$group_data) {
	foreach ($group_data['rule'] as $rule_key=>$rule_value) {
		$this->parseElementPHP($ddm_group, 'datatableajax', $this->getSendElementValue($ddm_group, $rule_key, 'module'), ['module'=>$this->getSendElementValue($ddm_group, $rule_key, 'module'), 'rule_key'=>$rule_key, 'rule_value'=>$rule_value]);
