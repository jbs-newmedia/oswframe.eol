	...
	<tr class="table_ddm_row table_ddm_row_data <?php echo osW_Template::getInstance()->getColorClass('table_ddm_rows', ['table_ddm_row_cella', 'table_ddm_row_cellb']) ?> <?php if (osW_Form::getInstance()->getFormError($element)===true): ?>table_ddm_row_error<?php endif ?> ddm_element_<?php echo $this->getSendElementValue($ddm_group, $element, 'id') ?> ">
		...
		<td class="table_ddm_col table_ddm_col_data table_ddm_col_form">
			...
			<?php if ($this->getDoSendElementStorage($ddm_group, $element.$this->getSendElementOption($ddm_group, $element, 'temp_suffix'))!=''): ?>
				...
				<br/>
				<?php osW_Form::getInstance()->drawHiddenField($element.$this->getSendElementOption($ddm_group, $element, 'temp_suffix'), $this->getDoSendElementStorage($ddm_group, $element.$this->getSendElementOption($ddm_group, $element, 'temp_suffix'))) ?><?php osW_Form::getInstance()->drawHiddenField($element, $this->getDoSendElementStorage($ddm_group, $element)) ?><?php elseif ($this->getSendElementStorage($ddm_group, $element)!=''): ?>


				<a target="_blank" href="<?php echo $this->getSendElementStorage($ddm_group, $element) ?>"><?php echo h()->_outputString($this->getSendElementOption($ddm_group, $element, 'text_file_view')) ?></a><?php if ($this->getSendElementOption($ddm_group, $element, 'edit_enabled')): ?> |
				...
				<br/>
				<?php osW_Form::getInstance()->drawHiddenField($element, $this->getSendElementStorage($ddm_group, $element)) ?><?php endif ?>

			<?php echo osW_Form::getInstance()->drawFileField($element, $this->getSendElementStorage($ddm_group, $element), []) ?>

			<?php if ($this->getDoSendElementStorage($ddm_group, $element.$this->getSendElementOption($ddm_group, $element, 'temp_suffix'))!=''): ?>
				<br/><?php echo osW_Form::getInstance()->drawCheckboxField($element.$this->getSendElementOption($ddm_group, $element, 'temp_suffix').$this->getSendElementOption($ddm_group, $element, 'delete_suffix'), 1, 0, []) ?> Datei löschen
				<br/>
			<?php elseif ($this->getSendElementStorage($ddm_group, $element)!=''): ?>
				<br/><?php echo osW_Form::getInstance()->drawCheckboxField($element.$this->getSendElementOption($ddm_group, $element, 'delete_suffix'), 1, 0, []) ?><?php echo h()->_outputString($this->getSendElementOption($ddm_group, $element, 'text_file_delete')) ?>
				<br/>
			<?php endif ?>
