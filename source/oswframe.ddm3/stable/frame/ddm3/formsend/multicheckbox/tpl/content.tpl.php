...
?>
	...
	<tr class="table_ddm_row table_ddm_row_data <?php echo osW_Template::getInstance()->getColorClass('table_ddm_rows', ['table_ddm_row_cella', 'table_ddm_row_cellb']) ?> <?php if (osW_Form::getInstance()->getFormError($element)===true): ?>table_ddm_row_error<?php endif ?> ddm_element_<?php echo $this->getSendElementValue($ddm_group, $element, 'id') ?> ">
		...
		<td class="table_ddm_col table_ddm_col_data table_ddm_col_form">
			<?php $multicheckbox=[] ?>
			<?php if (strlen($this->getSendElementStorage($ddm_group, $element))>0): ?><?php $multicheckbox=explode($this->getSendElementOption($ddm_group, $element, 'separator'), $this->getSendElementStorage($ddm_group, $element)) ?><?php endif ?>
			<ul class="table_ddm_list <?php if ($this->getSendElementOption($ddm_group, $element, 'orientation')=='horizontal'): ?><?php echo ' table_ddm_list_horizontal' ?><?php else: ?><?php echo 'table_ddm_list_vertical' ?><?php endif ?>">
				<?php foreach ($this->getSendElementOption($ddm_group, $element, 'data') as $key=>$value): ?>
					<li>
						<?php if (in_array($key, $multicheckbox)): ?><?php echo osW_Form::getInstance()->drawCheckboxField($element.'_'.$key, '1', 1, []) ?><?php echo h()->_outputString($value) ?><?php else: ?><?php echo osW_Form::getInstance()->drawCheckboxField($element.'_'.$key, '1', 0, []) ?><?php echo h()->_outputString($value) ?><?php endif ?>
					</li>
				<?php endforeach; ?>
			</ul>
		</td>
