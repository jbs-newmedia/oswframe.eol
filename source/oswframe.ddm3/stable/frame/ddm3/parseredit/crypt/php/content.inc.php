...
if (strlen($this->getDoEditElementStorage($ddm_group, $element))>0) {
	...
	if (($this->getFilterErrorElementStorage($ddm_group, $element)!==true)&&($this->getEditElementValidation($ddm_group, $element, 'length_min')!='')) {
		if (strlen($this->getDoEditElementStorage($ddm_group, $element))<$this->getEditElementValidation($ddm_group, $element, 'length_min')) {
			osW_Form::getInstance()->addFormError($element);
			osW_MessageStack::getInstance()->add('form', 'error', ['msg'=>h()->_setText($this->getGroupMessage($ddm_group, 'validation_element_toshort'), $this->getFilterElementStorage($ddm_group, $element))]);
	...
	if (($this->getFilterErrorElementStorage($ddm_group, $element)!==true)&&($this->getEditElementValidation($ddm_group, $element, 'length_max')!='')) {
		if (strlen($this->getDoEditElementStorage($ddm_group, $element))>$this->getEditElementValidation($ddm_group, $element, 'length_max')) {
			osW_Form::getInstance()->addFormError($element);
			osW_MessageStack::getInstance()->add('form', 'error', ['msg'=>h()->_setText($this->getGroupMessage($ddm_group, 'validation_element_tolong'), $this->getFilterElementStorage($ddm_group, $element))]);
			$this->setFilterErrorElementStorage($ddm_group, $element, true);
		}
	}
	...
	$this->removeEditElementValue($ddm_group, $element, 'name');
}
