...
if (osW_Settings::getInstance()->getAction()=='dodelete') {
	...
	if ($this->getDeleteElementOption($ddm_group, $element, 'store_name')===true) {
		...
		osW_DDM3::getInstance()->addDataElement($ddm_group, $element.'_name', ['module'=>'hidden', 'name'=>$this->getDeleteElementValue($ddm_group, $element, 'name').'_name']);
	}

	if ($this->getDeleteElementOption($ddm_group, $element, 'store_type')===true) {
		$this->setDoDeleteElementStorage($ddm_group, $element.'_type', $data_old[$element.'_type']);
		osW_DDM3::getInstance()->addDataElement($ddm_group, $element.'_type', ['module'=>'hidden', 'name'=>$this->getDeleteElementValue($ddm_group, $element, 'name').'_type']);
	}

	if ($this->getDeleteElementOption($ddm_group, $element, 'store_size')===true) {
		$this->setDoDeleteElementStorage($ddm_group, $element.'_size', $data_old[$element.'_size']);
		osW_DDM3::getInstance()->addDataElement($ddm_group, $element.'_size', ['module'=>'hidden', 'name'=>$this->getDeleteElementValue($ddm_group, $element, 'name').'_size']);
	}

	if ($this->getDeleteElementOption($ddm_group, $element, 'store_md5')===true) {
		$this->setDoDeleteElementStorage($ddm_group, $element.'_md5', $data_old[$element.'_md5']);
		osW_DDM3::getInstance()->addDataElement($ddm_group, $element.'_md5', ['module'=>'hidden', 'name'=>$this->getDeleteElementValue($ddm_group, $element, 'name').'_md5']);
	}
}
