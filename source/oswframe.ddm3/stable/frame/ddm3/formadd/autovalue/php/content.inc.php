...
if (osW_Settings::getInstance()->getAction()=='doadd') {
	...
	if ($this->getAddElementOption($ddm_group, $element, 'filter_use')===true) {
	...
	if ($this->getAddElementOption($ddm_group, $element, 'default_value')!='') {
		$database_where_string.=' AND ('.$this->getAddElementValue($ddm_group, $element, 'name').'>='.$this->getAddElementOption($ddm_group, $element, 'default_value').')';
	}
	...
	$QcheckData->bindRaw(':formdata_name:', $this->getGroupOption($ddm_group, 'alias', 'database').'.'.$this->getAddElementValue($ddm_group, $element, 'name'));
	$QcheckData->bindRaw(':where:', $database_where_string);
	$QcheckData->execute();
	if ($QcheckData->numberOfRows()==1) {
		$QcheckData->next();
		$this->setDoAddElementStorage($ddm_group, $element, ($QcheckData->result[$this->getAddElementValue($ddm_group, $element, 'name')]+1));
	} else {
		$this->setDoAddElementStorage($ddm_group, $element, $this->getAddElementOption($ddm_group, $element, 'default_value'));
	}
}
