...
if ((osW_Settings::getInstance()->getAction()=='edit')||(osW_Settings::getInstance()->getAction()=='doedit')) {
	...
	$Qselect->bindRaw(':elements:', implode(', ', [$this->getGroupOption($ddm_group, 'alias', 'database').'.'.$this->getEditElementOption($ddm_group, $element, 'prefix').'create_time', $this->getGroupOption($ddm_group, 'alias', 'database').'.'.$this->getEditElementOption($ddm_group, $element, 'prefix').'create_user_id', $this->getGroupOption($ddm_group, 'alias', 'database').'.'.$this->getEditElementOption($ddm_group, $element, 'prefix').'update_time', $this->getGroupOption($ddm_group, 'alias', 'database').'.'.$this->getEditElementOption($ddm_group, $element, 'prefix').'update_user_id']));
	...
	if ($Qselect->numberOfRows()==1) {
		...
		if (osW_Settings::getInstance()->getAction()=='doedit') {
			...
			osW_DDM3::getInstance()->addDataElement($ddm_group, $this->getEditElementOption($ddm_group, $element, 'prefix').'update_time', ['module'=>'hidden', 'name'=>$this->getEditElementOption($ddm_group, $element, 'prefix').'update_time', 'options'=>['default_value'=>$this->getDoEditElementStorage($ddm_group, $element, $this->getEditElementOption($ddm_group, $element, 'prefix').'update_time')]]);
			osW_DDM3::getInstance()->addDataElement($ddm_group, $this->getEditElementOption($ddm_group, $element, 'prefix').'update_user_id', ['module'=>'hidden', 'name'=>$this->getEditElementOption($ddm_group, $element, 'prefix').'update_user_id', 'options'=>['default_value'=>$this->getDoEditElementStorage($ddm_group, $element, $this->getEditElementOption($ddm_group, $element, 'prefix').'update_user_id')]]);
		}
	}
}
