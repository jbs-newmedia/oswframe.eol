...
?>
	...
	<tr class="table_ddm_row table_ddm_row_data <?php echo osW_Template::getInstance()->getColorClass('table_ddm_rows', ['table_ddm_row_cella', 'table_ddm_row_cellb']) ?> <?php if (osW_Form::getInstance()->getFormError($element)===true): ?>table_ddm_row_error<?php endif ?> ddm_element_<?php echo $this->getEditElementValue($ddm_group, $element, 'id') ?> ">
		...
		<td class="table_ddm_col table_ddm_col_data table_ddm_col_form">
			<?php if ($this->getEditElementOption($ddm_group, $element, 'read_only')===true): ?><?php echo osW_Form::getInstance()->drawHiddenField($element, $this->getEditElementStorage($ddm_group, $element), []) ?><?php if ($this->getEditElementStorage($ddm_group, $element)==1): ?><?php echo h()->_outputString($values['options']['text_yes']) ?><?php else: ?><?php echo h()->_outputString($values['options']['text_no']) ?><?php endif ?><?php else: ?>
				<ul class="table_ddm_list table_ddm_list_horizontal">
					<?php if ($this->getEditElementOption($ddm_group, $element, 'displayorder')=='yn'): ?>
						<li><?php echo osW_Form::getInstance()->drawRadioField($element, '1', $this->getEditElementStorage($ddm_group, $element), []) ?><?php echo h()->_outputString($values['options']['text_yes']) ?></li>
						<li><?php echo osW_Form::getInstance()->drawRadioField($element, '0', $this->getEditElementStorage($ddm_group, $element), []) ?><?php echo h()->_outputString($values['options']['text_no']) ?></li>
					<?php else: ?>
						<li><?php echo osW_Form::getInstance()->drawRadioField($element, '0', $this->getEditElementStorage($ddm_group, $element), []) ?><?php echo h()->_outputString($values['options']['text_no']) ?></li>
						<li><?php echo osW_Form::getInstance()->drawRadioField($element, '1', $this->getEditElementStorage($ddm_group, $element), []) ?><?php echo h()->_outputString($values['options']['text_yes']) ?></li>
					<?php endif ?>
