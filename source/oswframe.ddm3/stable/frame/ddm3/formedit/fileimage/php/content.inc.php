...
if (osW_Settings::getInstance()->getAction()=='doedit') {
	...
	if (($this->getEditElementOption($ddm_group, $element, 'store_name')===true)||($this->getEditElementOption($ddm_group, $element, 'store_type')===true)||($this->getEditElementOption($ddm_group, $element, 'store_size')===true)||($this->getEditElementOption($ddm_group, $element, 'store_md5')===true)) {
		...
		$Qselect->bindRaw(':elements:', implode(', ', [$this->getGroupOption($ddm_group, 'alias', 'database').'.'.$element.'_name', $this->getGroupOption($ddm_group, 'alias', 'database').'.'.$element.'_type', $this->getGroupOption($ddm_group, 'alias', 'database').'.'.$element.'_size', $this->getGroupOption($ddm_group, 'alias', 'database').'.'.$element.'_md5']));
	...
	if ((isset($_FILES[$element]))&&($_FILES[$element]['error']==0)) {
		...
		if ($this->getEditElementOption($ddm_group, $element, 'store_name')===true) {
			...
			osW_DDM3::getInstance()->addDataElement($ddm_group, $element.'_name', ['module'=>'hidden', 'name'=>$this->getEditElementValue($ddm_group, $element, 'name').'_name']);
		}

		if ($this->getEditElementOption($ddm_group, $element, 'store_type')===true) {
			$this->setDoEditElementStorage($ddm_group, $element.'_type', ($_FILES[$element]['type']));
			osW_DDM3::getInstance()->addDataElement($ddm_group, $element.'_type', ['module'=>'hidden', 'name'=>$this->getEditElementValue($ddm_group, $element, 'name').'_type']);
		}

		if ($this->getEditElementOption($ddm_group, $element, 'store_size')===true) {
			$this->setDoEditElementStorage($ddm_group, $element.'_size', ($_FILES[$element]['size']));
			osW_DDM3::getInstance()->addDataElement($ddm_group, $element.'_size', ['module'=>'hidden', 'name'=>$this->getEditElementValue($ddm_group, $element, 'name').'_size']);
		}

		if ($this->getEditElementOption($ddm_group, $element, 'store_md5')===true) {
			$this->setDoEditElementStorage($ddm_group, $element.'_md5', md5_file($_FILES[$element]['tmp_name']));
			osW_DDM3::getInstance()->addDataElement($ddm_group, $element.'_md5', ['module'=>'hidden', 'name'=>$this->getEditElementValue($ddm_group, $element, 'name').'_md5']);
		}
...
if ($this->getEditElementOption($ddm_group, $element, 'edit_enabled')) {
	osW_JQuery2::getInstance()->loadPlugin('fancybox2');

	osW_Template::getInstance()->addJSCodeHead('
if (!saved) {
	var saved = [];
}
saved["'.$ddm_group.'_'.$element.'"]=false;

$(function() {
	$("#ddm_element_'.$ddm_group.'_'.$element.'_crop_link").fancybox({
		width:800,
		height:600,
		autoSize: false,
		...
		beforeClose: function() {
			if (saved["'.$ddm_group.'_'.$element.'"]===true) {
				var strconfirm = confirm("Fenster ohne Speichern schließen?");
				if (strconfirm !== true) {
			    	return false;
    			}
			}
...
');
	...
	osW_Session::getInstance()->set($ddm_group.'_'.$element, ['element'=>$this->getEditElement($ddm_group, $element), 'storage'=>$this->getEditElementStorage($ddm_group, $element), 'ddm_group'=>$ddm_group, 'ddm_element'=>$element, 'layout'=>$this->getGroupOption($ddm_group, 'layout')]);
}
