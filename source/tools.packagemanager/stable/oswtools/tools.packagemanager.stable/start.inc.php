...
<div class="container">
	...
	<table id="oswtools_manager" class="table table-striped table-bordered">
		...
		<?php if (($sl=='')&&(count($packages)>1)): ?><?php foreach ($packages as $sl=>$foo): ?><?php $serverlist_data=osW_Tool_Server::getInstance()->readServerList($sl); ?><?php $i=0;
			foreach (osW_Tool_PackageManager::getInstance()->checkPackageList($packages[$sl]) as $package_name=>$package_data):$i++; ?>
			...
			foreach ($package_installed as $package_name=>$package_data):$i++; ?>
			...
			<?php endforeach ?><?php elseif (count(osW_Tool_PackageManager::getInstance()->checkPackageList($packages[$sl]))>0): ?><?php $i=0;
			foreach (osW_Tool_PackageManager::getInstance()->checkPackageList($packages[$sl]) as $package_name=>$package_data):$i++; ?>
				<tr id="package_<?php echo md5($sl.'#'.$package_data['package'].'#'.$package_data['release']) ?>">
					<td><?php echo $package_data['info']['name']; ?></td>
					<td><?php echo $package_data['release']; ?></td>
