	...
	<ul class="nav nav-tabs">
		<?php foreach ($data as $serverlist=>$serverlist_packages): ?>
			<li<?php if ($sl==$serverlist): ?> class="active"<?php endif ?>>
				<a href="index.php?session=<?php echo osW_Tool_Session::getInstance()->getId() ?>&action=<?php echo osW_Tool::getInstance()->getAction() ?>&sl=<?php echo $serverlist ?>"><?php echo $serverlist_packages['server_list'] ?></a>
			</li>
		<?php endforeach ?>
	</ul>
