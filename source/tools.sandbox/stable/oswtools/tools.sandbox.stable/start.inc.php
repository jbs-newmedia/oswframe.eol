...
<div class="container">
	...
	<form id="sandbox_start" method="post" action="index.php?session=<?php echo osW_Tool_Session::getInstance()->getId() ?>&action=start">
		...
		<div class="row">
			...
			<div class="col-xs-4 col-sm-2">
				<button type="button" class="btn btn-primary btn-block" onclick="$('#sandbox_start').submit()" form="chmod_start">Run</button>
			</div>
		</div>
	...
	<?php if (($sandbox_file!='')&&(isset($ar_sandbox_files[$sandbox_file]))): ?><?php include abs_path.'resources/php/sandbox/'.$sandbox_file ?><?php endif ?>
