...
<div class="container">
	<table id="oswtools_main" class="table table-striped table-bordered">
		...
		<tbody>
		<?php if ($manager_tools!=[]): ?><?php foreach ($manager_tools as $serverlist=>$tools): ?><?php $i=1;
			foreach ($tools as $package_name=>$package_data):$i++; ?>
				<tr>
					<td>
						<a href="../<?php echo $package_name ?>/index.php?&session=<?php echo osW_Tool_Session::getInstance()->getId() ?>"><?php if (isset($package_data['info']['name'])): ?><?php echo $package_data['info']['name']; ?><?php else: ?><?php echo $package_name; ?><?php endif ?></a>
					</td>
