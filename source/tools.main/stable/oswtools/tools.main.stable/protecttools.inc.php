...
<div class="container">
	...
	<ul class="nav nav-tabs">
		<li<?php if ($part=='new'): ?> class="active"<?php endif ?>>
			<a href="index.php?session=<?php echo osW_Tool_Session::getInstance()->getId() ?>&action=<?php echo osW_Tool::getInstance()->getAction() ?>&part=new">New User</a>
		</li>
		<li<?php if ($part=='manage'): ?> class="active"<?php endif ?>>
			<a href="index.php?session=<?php echo osW_Tool_Session::getInstance()->getId() ?>&action=<?php echo osW_Tool::getInstance()->getAction() ?>&part=manage">Manage User</a>
		</li>
	</ul>
		...
		if (osW_Tool::getInstance()->getDoAction()=='donew') {
			...
			if (count($error)==0) {
				...
				if ((!file_exists($config['htaccess']))||(!strstr($htaccess, '#osWFrame tpf#'))) {
					...
					$rules=['#osWFrame tpf#', 'AuthType Basic', 'AuthName "osWTools"', 'AuthUserFile "'.$config['htpasswd'].'"', 'require valid-user'];
		...
		<form id="protecttools_new" method="post" action="index.php?session=<?php echo osW_Tool_Session::getInstance()->getId() ?>&action=protecttools&part=new">
			...
			<div class="form-group<?php if (isset($error['htaccess_user'])): ?> has-error<?php endif ?>">
				...
				<div>
					...
					<?php if (isset($error['htaccess_user'])): ?>
						<span class="help-block"><?php echo $error['htaccess_user'] ?></span><?php endif ?>
				</div>
			</div>
			...
			<div class="form-group<?php if (isset($error['htaccess_password'])): ?> has-error<?php endif ?>">
				...
				<div>
					...
					<?php if (isset($error['htaccess_password'])): ?>
						<span class="help-block"><?php echo $error['htaccess_password'] ?></span><?php endif ?>
				</div>
			</div>
			...
			<div class="form-group<?php if (isset($error['htaccess_confirm'])): ?> has-error<?php endif ?>">
				...
				<div>
					...
					<?php if (isset($error['htaccess_confirm'])): ?>
						<span class="help-block"><?php echo $error['htaccess_confirm'] ?></span><?php endif ?>
				</div>
			</div>
	...
	if ($part=='manage') {
		...
		<form id="protecttools_manage" method="post" action="index.php?session=<?php echo osW_Tool_Session::getInstance()->getId() ?>&action=protecttools&part=manage">
			...
			<table id="oswtools_main_protecttools_manager" class="table table-striped table-bordered">
				...
				<tbody>
				<?php if (count($htpasswd)>0): ?><?php $i=1;
					foreach ($users as $user=>$blank):$i++ ?>
						<tr>
							<td><?php echo outputString($user) ?></td>
							<td>
								<label class="checkbox-inline"><input type="checkbox" name="updtusers[<?php echo $user; ?>]" value="1"/> remove</label>
							</td>
						</tr>
