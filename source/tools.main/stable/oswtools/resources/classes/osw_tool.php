...
class osW_Tool extends osW_Tool_Object {
	...
	public function getUpdateVersion($serverlist='oswframe') {
		$file=abs_path.'resources/json/package/'.package.'-'.release.'.json';
		if (file_exists($file)) {
			$info=json_decode(file_get_contents($file), true);
	...
	public function checkUpdate($serverlist='oswframe') {
		$file=abs_path.'resources/json/package/'.package.'-'.release.'.json';
		if (file_exists($file)) {
			$info=json_decode(file_get_contents($file), true);
			$server_data=osW_Tool_Server::getInstance()->getConnectedServer($serverlist);
	...
	public function update($serverlist='oswframe') {
		$file=abs_path.'resources/json/package/'.package.'-'.release.'.json';
		if (file_exists($file)) {
			$info=json_decode(file_get_contents($file), true);
			$server_data=osW_Tool_Server::getInstance()->getConnectedServer($serverlist);
	...
	public function installPackageForce($package, $release, $serverlist='oswframe', $file='', $dir='') {
		$return=true;
		$server_data=osW_Tool_Server::getInstance()->getConnectedServer($serverlist);
		if ((isset($server_data['connected']))&&($server_data['connected']===true)) {
	...
	public function installPackage($package, $release, $serverlist='oswframe', $file='', $dir='') {
		$server_data=osW_Tool_Server::getInstance()->getConnectedServer($serverlist);
		if ((isset($server_data['connected']))&&($server_data['connected']===true)) {
			$return=true;
			...
			$this->data['package_proceed'][]=array('package'=>$package, 'release'=>$release, 'serverlist'=>$serverlist);
		} else {
			$return=false;
		}
		return $return;
	...
	public function createConfigureFile() {
		...
		if (!empty($configure)) {
			...
			$configure_output[]=' * @copyright Copyright (c), Juergen Schwind';
			$configure_output[]=' * @package osWFrame';
			$configure_output[]=' * @version '.date('YmdHis').' (created by oswtools)';
			$configure_output[]=' * @link http://oswframe.com';
			$configure_output[]=' * @license http://www.gnu.org/licenses/gpl.html GNU General Public License';
			$configure_output[]=' *';
			$configure_output[]=' */';
			$configure_output[]='';
			...
			foreach (array('top','topmiddle','middle','middlebottom','bottom') as $part) {
				foreach ($configure as $configure_file=>$configure_data) {
					if (isset($configure_data['configure']['configure'][$part])) {
						...
						foreach ($configure_data['configure']['configure'][$part] as $key=>$value) {
							...
							if (is_bool($value)) {
								if ($value===true) {
									$configure_output[]='osW_Settings::getInstance()->'.$key.'=true;';
								} else {
									$configure_output[]='osW_Settings::getInstance()->'.$key.'=false;';
								}
							} elseif (is_numeric($value)) {
								$configure_output[]='osW_Settings::getInstance()->'.$key.'='.str_replace(',', '.', $value).';';
							} else {
								$configure_output[]='osW_Settings::getInstance()->'.$key.'=\''.$value.'\';';
							}
						}
						$configure_output[]='';
					}
	...
	public function createHtAccessFile() {
		$configure_files=array(root_path.'frame/configure.php',root_path.'modules/configure.project.php',root_path.'modules/configure.project-dev.php');
		...
		foreach ($configure_files as $configure_file) {
			if (file_exists($configure_file)) {
				...
				$content=preg_replace('/osW_Settings\:\:getInstance\(\)-\>([a-zA-Z0-9_]+) *=/', '$vars[\'$1\']=', $content);
				eval(substr($content, 5));
			}
		}

		if (isset($vars['project_path'])&&($vars['project_path']!='')) {
			$vars['project_url_path']='/'.$vars['project_path'].'/';
		} else {
			$vars['project_url_path']='/';
		}
	...
	public function loadFrameConfig() {
		if (!isset($this->data['configure'])) {
			...
			$this->data['configure']=array();
			foreach ($configure_files as $configure_file) {
				if (file_exists($configure_file)) {
					$content=file_get_contents($configure_file);
					$content=str_replace('settings_abspath', 'abs_path', $content);
					$content=preg_replace('/osW_Settings\:\:getInstance\(\)-\>([a-zA-Z0-9_]+) *=/', '$this->data[\'configure\'][\'$1\']=', $content);
					eval(substr($content, 5));
				}
			}
		}
	...
	public function getLicense($license) {
		switch ($license) {
			case 'GNU General Public License':
				return str_replace(array('<a name="', '<a ', '<a target="_blank" id="'), array('<a id="', '<a target="_blank" ', '<a id="'), file_get_contents(abs_path.'resources/license/gpl-3.0.html'));
				break;
			default:
