...
if (isset($settings['ignore'])) {
	...
	$clear_list=osW_Tool_ProjectClear::getInstance()->getList([]);
}
...
<div class="container">
	...
	<form id="oswtools_projectclear" method="post" action="index.php?session=<?php echo osW_Tool_Session::getInstance()->getId() ?>&action=start">
		...
		<table id="frameclear_main" class="table table-striped table-bordered">
			...
			<?php if (count($clear_list)>0): ?><?php $i=0;
				foreach ($clear_list as $element=>$status):$i++; ?><?php if (in_array($status, [2])): ?>
					<tr>
						<td class="text-center">
							<input type="checkbox" name="element[<?php echo $element ?>]" value="1"/></td>
						<td><?php echo $element ?>
							<div class="pull-right">
								<a title="Ignore" href="javascript:engine('<?php echo $element ?>', '<?php echo osW_Tool_Session::getInstance()->getId() ?>')" class="remove buttonosw"><i class="fa fa-remove fa-fw"></i></a>
							</div>
						</td>
					</tr>
