 ...
 *
 */
class osW_Database_mysql extends osW_Database_Engine {

	/* METHODS CORE */
	public function __construct($alias, $options) {
		parent::__construct($alias, $options, __CLASS__, 2, 1);
	}
	...
	public function _init() {
		// set flags
		$setUTF8=$this->query('SET character_set_client="'.vOut('database_character').'"');
		$setUTF8->execute();
		$setUTF8=$this->query('SET character_set_connection="'.vOut('database_character').'"');
		$setUTF8->execute();
		$setUTF8=$this->query('SET character_set_results="'.vOut('database_character').'"');
		$setUTF8->execute();
