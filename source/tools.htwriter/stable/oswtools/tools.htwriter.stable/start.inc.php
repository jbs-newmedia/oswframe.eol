	...
	if (osW_Tool::getInstance()->getDoAction()=='doit') {
		...
		if (count($error)==0) {
			...
			if ((!file_exists($config['htwriter']))||(!strstr($htaccess, '#osWFrame tpf#'))) {
				...
				$rules=['#osWFrame tpf#', 'AuthType Basic', 'AuthName "Login osWTools"', 'AuthUserFile "'.$config['htpasswd'].'"', 'require valid-user'];
	...
	<form id="htwriter_new" method="post" action="index.php?session=<?php echo osW_Tool_Session::getInstance()->getId() ?>&action=start">
		...
		<div class="form-group<?php if (isset($error['htwriter_directory'])): ?> has-error<?php endif ?>">
			...
			<div>
				...
				<?php if (isset($error['htwriter_directory'])): ?>
					<span class="help-block"><?php echo $error['htwriter_directory'] ?></span><?php endif ?>
			</div>
		</div>
		...
		<div class="form-group<?php if (isset($error['htwriter_user'])): ?> has-error<?php endif ?>">
			...
			<div>
				...
				<?php if (isset($error['htwriter_user'])): ?>
					<span class="help-block"><?php echo $error['htwriter_user'] ?></span><?php endif ?>
			</div>
		</div>
		...
		<div class="form-group<?php if (isset($error['htwriter_password'])): ?> has-error<?php endif ?>">
			...
			<div>
				...
				<?php if (isset($error['htwriter_password'])): ?>
					<span class="help-block"><?php echo $error['htwriter_password'] ?></span><?php endif ?>
			</div>
		</div>
		...
		<div class="form-group<?php if (isset($error['htwriter_confirm'])): ?> has-error<?php endif ?>">
			...
			<div>
				...
				<?php if (isset($error['htwriter_confirm'])): ?>
					<span class="help-block"><?php echo $error['htwriter_confirm'] ?></span><?php endif ?>
			</div>
		</div>
