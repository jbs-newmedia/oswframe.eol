...
<div class="container">
	...
	<form name="serverlist" method="post" action="index.php?session=<?php echo osW_Tool_Session::getInstance()->getId() ?>&action=start">
		...
		<?php if (!empty($config['message'])): ?><?php osW_Tool_Template::getInstance()->outputB3Alerts(['success'=>[$config['message']]]) ?><?php endif ?>
		...
		<div class="form-group<?php if (($config['error']!='')): ?> has-error<?php endif ?>">
			...
			<div>
				...
				<?php if (($config['error']!='')): ?>
					<span class="help-block"><?php echo $config['error'] ?></span><?php endif ?>
			</div>
		</div>
