...
<div class="container">
	...
	<?php if ($ar_patcher_files!=[]): ?>
		...
		<form id="patcher_start" method="post" action="index.php?session=<?php echo osW_Tool_Session::getInstance()->getId() ?>&action=start">
			...
			<div class="row">
				...
				<div class="col-xs-4 col-sm-2">
					<button type="button" class="btn btn-primary btn-block" onclick="$('#patcher_start').submit()" form="chmod_start">Run</button>
				</div>
			</div>
		...
		<?php if (($patcher_file!='')&&(isset($ar_patcher_files[$patcher_file]))): ?><?php include abs_path.'resources/php/patcher/'.$patcher_file ?><?php endif ?>
