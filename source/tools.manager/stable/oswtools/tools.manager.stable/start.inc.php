...
foreach (osW_Tool_Server::getInstance()->getPackageList() as $serverlist=>$server_packages) {
	foreach (osW_Tool_Manager::getInstance()->checkPackageList($server_packages) as $package_name=>$package_data) {
...
if (count($packages)==1) {
	...
	if ((!isset($packages[$sl]))&&($custom_tools==[]&&($sl!='custom'))) {
		$sl='oswframe';
	}
} else {
...
if ($doaction=='remove') {
	...
	ob_clean();
	$data=osW_Tool_Server::getInstance()->checkPackageList($packages[$sl]);
	die(json_encode([md5($sl.'#'.$manager_package.'#'.$manager_release)=>$data[$manager_package.'-'.$manager_release]]));
}

if ($doaction=='removecustom') {
	osW_Tool_Manager::getInstance()->removeCustomPackage($manager_package);
	ob_clean();
	die(json_encode([md5($sl.'#'.$manager_package)=>'removed']));
}
...
<div class="container">
	...
	<ul class="nav nav-tabs">
		<?php if (count($packages)>1): ?>
			<li<?php if ($sl==''): ?> class="active"<?php endif ?>>
				<a href="index.php?session=<?php echo osW_Tool_Session::getInstance()->getId() ?>&action=<?php echo osW_Tool::getInstance()->getAction() ?>&sl=">*</a>
			</li>
		<?php endif ?>
		<?php foreach ($packages as $serverlist=>$server_packages): ?><?php $serverlist_data=osW_Tool_Server::getInstance()->readServerList($serverlist); ?>
			<li<?php if ($sl==$serverlist): ?> class="active"<?php endif ?>>
				<a href="index.php?session=<?php echo osW_Tool_Session::getInstance()->getId() ?>&action=<?php echo osW_Tool::getInstance()->getAction() ?>&sl=<?php echo $serverlist ?>"><?php echo $serverlist_data['info']['name'] ?></a>
			</li>
		<?php endforeach ?>
		<?php if ($custom_tools!=[]): ?>
			<li<?php if ($sl=='custom'): ?> class="active"<?php endif ?>>
				<a href="index.php?session=<?php echo osW_Tool_Session::getInstance()->getId() ?>&action=<?php echo osW_Tool::getInstance()->getAction() ?>&sl=custom">Custom</a>
			</li>
		<?php endif ?>
	...
	<table id="oswtools_manager" class="table table-striped table-bordered">
		...
		<tbody>
		<?php if (($sl=='')&&(count($packages)>1)): ?><?php foreach ($packages as $sl=>$foo): ?><?php $serverlist_data=osW_Tool_Server::getInstance()->readServerList($sl); ?><?php $i=0;
			foreach (osW_Tool_Manager::getInstance()->checkPackageList($packages[$sl]) as $package_name=>$package_data):$i++; ?>
				<tr id="package_<?php echo md5($sl.'#'.$package_data['package'].'#'.$package_data['release']) ?>">
					<td class="manager_link"><?php if (($package_data['options']['install']==false)&&(file_exists('../'.$package_data['package'].'.'.$package_data['release'].'/index.php'))): ?>
							<a href="../<?php echo $package_data['package']; ?>.<?php echo $package_data['release']; ?>/index.php?=<?php echo osW_Tool_Session::getInstance()->getId() ?>"><?php echo $package_data['info']['name']; ?></a><?php else: ?><?php echo $package_data['info']['name']; ?><?php endif ?>
					</td>
			...
			<?php endforeach ?><?php endforeach ?><?php elseif ($sl=='custom'): ?><?php $i=0;
			foreach ($custom_tools as $package_name=>$package_data):$i++; ?>
				<tr id="package_<?php echo md5($sl.'#'.$package_name) ?>">
					<td class="manager_link">
						<a href="../<?php echo $package_name; ?>/index.php?=<?php echo osW_Tool_Session::getInstance()->getId() ?>"><?php echo $package_name; ?></a>
					</td>
					<td class="manager_options text-center">
						<a title="Remove" href="javascript:removeCustom('<?php echo md5($sl.'#'.$package_name) ?>', '<?php echo osW_Tool_Session::getInstance()->getId() ?>', '<?php echo $package_name ?>')" class="remove buttonosw"><i class="fa fa-remove fa-fw"></i></a>
					</td>
				</tr>
			<?php endforeach ?><?php elseif (count(osW_Tool_Manager::getInstance()->checkPackageList($packages[$sl]))>0): ?><?php $i=0;
			foreach (osW_Tool_Manager::getInstance()->checkPackageList($packages[$sl]) as $package_name=>$package_data):$i++; ?>
				<tr id="package_<?php echo md5($sl.'#'.$package_data['package'].'#'.$package_data['release']) ?>">
					<td class="manager_link"><?php if (($package_data['options']['install']==false)&&(file_exists('../'.$package_data['package'].'.'.$package_data['release'].'/index.php'))): ?>
							<a href="../<?php echo $package_data['package']; ?>.<?php echo $package_data['release']; ?>/index.php?=<?php echo osW_Tool_Session::getInstance()->getId() ?>"><?php echo $package_data['info']['name']; ?></a><?php else: ?><?php echo $package_data['info']['name']; ?><?php endif ?>
	...
	<?php if ($sl!='custom'): ?>
		...
		<div class="form-group ">
			<button type="button" class="btn btn-primary btn-block" onclick="updateAll()" form="oswtools_manager">Update all tools</button>
		</div>
	<?php endif ?>
