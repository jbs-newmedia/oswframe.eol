...
$ajax=$this->getDeleteElementOption($ddm_group, $element, 'data');
...
foreach (osW_DDM4::getInstance()->getDeleteElements($ddm_group) as $element => $options) {
...
foreach ($ajax['logic'] as $group => $group_data) {
	...
	foreach ($group_data['rule'] as $rule_key => $rule_values) {
		...
		foreach ($rule_values as $rule_value) {
			...
			if ((string)$this->getDeleteElementStorage($ddm_group, $rule_key)==(string)$rule_value) {
				$_rule=true;
			}
		}
...
foreach ($ajax['logic'] as $group => $group_data) {
	foreach ($group_data['rule'] as $rule_key => $rule_value) {
		$this->parseElementPHP($ddm_group, 'datatableajax', $this->getDeleteElementValue($ddm_group, $rule_key, 'module'), array('module'=>$this->getDeleteElementValue($ddm_group, $rule_key, 'module'), 'rule_key'=>$rule_key, 'rule_value'=>$rule_value));
