...
<tr class="table_ddm_row table_ddm_row_data <?php echo osW_Template::getInstance()->getColorClass('table_ddm_rows', array('table_ddm_row_cella', 'table_ddm_row_cellb'))?> <?php if(osW_Form::getInstance()->getFormError($element)===true):?>table_ddm_row_error<?php endif?> ddm_element_<?php echo $this->getDeleteElementValue($ddm_group, $element, 'id')?> ">
	...
	<td class="table_ddm_col table_ddm_col_data table_ddm_col_form">
		<?php if(($this->getDeleteElementStorage($ddm_group, $element)=='')||($this->getDeleteElementStorage($ddm_group, $element)=='00000000')):?>
			---
		<?php else:?>
			<?php if($this->getDeleteElementOption($ddm_group, $element, 'month_asname')===true):?>
				<?php echo strftime(str_replace('%m.', ' %B ', $this->getDeleteElementOption($ddm_group, $element, 'date_format')), mktime(12, 0, 0, substr($this->getDeleteElementStorage($ddm_group, $element), 4, 2), substr($this->getDeleteElementStorage($ddm_group, $element), 6, 2), substr($this->getDeleteElementStorage($ddm_group, $element), 0, 4)))?>
			<?php else:?>
				<?php echo strftime($this->getDeleteElementOption($ddm_group, $element, 'date_format'), mktime(12, 0, 0, substr($this->getDeleteElementStorage($ddm_group, $element), 4, 2), substr($this->getDeleteElementStorage($ddm_group, $element), 6, 2), substr($this->getDeleteElementStorage($ddm_group, $element), 0, 4)))?>
			<?php endif?>
		<?php endif?>
	</td>
</tr>