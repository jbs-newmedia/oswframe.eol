...
if (osW_Settings::getInstance()->getAction()=='dosend') {
	...
	if ((isset($_FILES[$element]))&&($_FILES[$element]['error']==0)) {
		...
		if ($this->getSendElementOption($ddm_group, $element, 'store_name')===true) {
			...
			osW_DDM4::getInstance()->addSendElement($ddm_group, $element.'_name', array(
				'module'=>'hidden',
				'name'=>$this->getSendElementValue($ddm_group, $element, 'name').'_name',
			));
		}
		...
		if ($this->getSendElementOption($ddm_group, $element, 'store_type')===true) {
			...
			osW_DDM4::getInstance()->addSendElement($ddm_group, $element.'_type', array(
				'module'=>'hidden',
				'name'=>$this->getSendElementValue($ddm_group, $element, 'name').'_type',
			));
		}
		...
		if ($this->getSendElementOption($ddm_group, $element, 'store_size')===true) {
			...
			osW_DDM4::getInstance()->addSendElement($ddm_group, $element.'_size', array(
				'module'=>'hidden',
				'name'=>$this->getSendElementValue($ddm_group, $element, 'name').'_size',
			));
		}
		...
		if ($this->getSendElementOption($ddm_group, $element, 'store_md5')===true) {
			...
			osW_DDM4::getInstance()->addSendElement($ddm_group, $element.'_md5', array(
				'module'=>'hidden',
				'name'=>$this->getSendElementValue($ddm_group, $element, 'name').'_md5',
			));
		}
