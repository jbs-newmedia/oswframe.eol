...
switch (osW_Settings::getInstance()->getAction()) {
	...
	case 'log_ajax':
		...
		$this->addDataElement($ddm_group, 'log_id', array(
			'module'=>'index',
			'title'=>'ID',
			'name'=>'log_id',
		));
...
if (in_array(osW_Settings::getInstance()->getAction(), array('ajax', 'log_ajax'))) {
			...
			if ((isset($columns[$order[$key]['column']]))&&(in_array($columns[$order[$key]['column']]['data'], $vars))) {
				if (isset($ddm_order_case_array[$this->getGroupOption($ddm_group, 'alias', 'database').'.'.$columns[$order[$key]['column']]['data']])) {
					...
					if ($order[$key]['dir']=='asc') {
						...
						foreach(array_reverse($ddm_order_case_array[$this->getGroupOption($ddm_group, 'alias', 'database').'.'.$columns[$order[$key]['column']]['data']]) as $k => $v){
							$i++;
							$sql.='WHEN '.h()->_escapeString($k).' THEN '.$i.' ';
						}
					}
	...
	// load list
	$QgetData=osW_Database::getInstance()->query('SELECT :vars: FROM :table: AS :alias: WHERE :search: :order:');
	...
	if ($QgetData->numberOfRows()>0) {
		...
		// output datatables-result
		h()->_die(json_encode(array('draw'=>h()->_catch('draw', 1, 'gp'), 'recordsTotal'=>0, 'recordsFiltered'=>0, 'data'=>array())));
	}
}
...
if (in_array(osW_Settings::getInstance()->getAction(), array('', 'log'))) {
	...
	foreach ($_columns as $element) {
		$ajax_columnDefs[] ='{ "data": "'.$element['name'].'", "orderable": '.(($element['order']===true)?'true':'false').', "searchable": '.(($element['search']===true)?'true':'false').', "targets": '.$i.' }';
