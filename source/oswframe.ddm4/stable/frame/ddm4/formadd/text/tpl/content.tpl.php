...
<div class="form-group<?php if(osW_Form::getInstance()->getFormError($element)):?> has-error<?php endif?> ddm_element_<?php echo $this->getAddElementValue($ddm_group, $element, 'id')?>">
	...
	<?php if($this->getAddElementOption($ddm_group, $element, 'read_only')===true):?>
		...
		<div class="form-control readonly"><?php echo h()->_outputString($this->getAddElementStorage($ddm_group, $element));?></div>
	<?php else:?>
		<?php echo osW_Form::getInstance()->drawTextField($element, $this->getAddElementStorage($ddm_group, $element), array('input_class'=>'form-control'));?>
	<?php endif?>
