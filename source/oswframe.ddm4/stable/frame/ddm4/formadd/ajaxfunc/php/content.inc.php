...
$ajax=$this->getAddElementOption($ddm_group, $element, 'data');
...
foreach (osW_DDM4::getInstance()->getAddElements($ddm_group) as $element => $options) {
...
if (osW_Settings::getInstance()->getAction()=='doadd') {
	foreach ($ajax['logic'] as $group => $group_data) {
		...
		foreach ($group_data['rule'] as $rule_key => $rule_values) {
			...
			foreach ($rule_values as $rule_value) {
				...
				if ((string)$this->getDoAddElementStorage($ddm_group, $rule_key)==(string)$rule_value) {
					$_rule=true;
				}
			}
	...
	foreach (osW_DDM4::getInstance()->getAddElements($ddm_group) as $element => $options) {
		if (!isset($elements_ok[$element])) {
			$this->setAddElementValidation($ddm_group, $element, 'module', 'ajaxfunc');
			// ???????
			$this->setAddElementValue($ddm_group, $element, 'name', '');
		}
	}
} else {
	foreach ($ajax['logic'] as $group => $group_data) {
		...
		foreach ($group_data['rule'] as $rule_key => $rule_values) {
			...
			foreach ($rule_values as $rule_value) {
				$ajax['logic'][$group]['jsrule'][$rule_key][]=$rule_value;
				if ((string)$this->getAddElementStorage($ddm_group, $rule_key)==(string)$rule_value) {
					$_rule=true;
				}
			}
...
foreach ($ajax['logic'] as $group => $group_data) {
	foreach ($group_data['rule'] as $rule_key => $rule_value) {
		$this->parseElementPHP($ddm_group, 'datatableajax', $this->getAddElementValue($ddm_group, $rule_key, 'module'), array('module'=>$this->getAddElementValue($ddm_group, $rule_key, 'module'), 'rule_key'=>$rule_key, 'rule_value'=>$rule_value));
	...
	foreach ($ajax['logic'][$group]['view'] as $element) {
		...
		if (isset($ajax['logic'][$group]['reset'])) {
			foreach ($ajax['logic'][$group]['reset'] as $reset_key) {
				$this->parseElementPHP($ddm_group, 'datatableajax', $this->getAddElementValue($ddm_group, $reset_key, 'module'), array('module'=>$this->getAddElementValue($ddm_group, $reset_key, 'module'), 'reset_key'=>$reset_key));
		...
		if (isset($ajax['logic'][$group]['set'])) {
			foreach ($ajax['logic'][$group]['set'] as $set_key => $set_value) {
				$this->parseElementPHP($ddm_group, 'datatableajax', $this->getAddElementValue($ddm_group, $set_key, 'module'), array('module'=>$this->getAddElementValue($ddm_group, $set_key, 'module'), 'set_key'=>$set_key, 'set_value'=>$set_value));
