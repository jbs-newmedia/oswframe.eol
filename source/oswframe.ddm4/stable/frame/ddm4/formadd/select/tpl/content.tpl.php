...
<div class="form-group<?php if(osW_Form::getInstance()->getFormError($element)):?> has-error<?php endif?> ddm_element_<?php echo $this->getAddElementValue($ddm_group, $element, 'id')?>">
	...
	<?php if($this->getAddElementOption($ddm_group, $element, 'read_only')===true):?>
		...
		<?php if($this->getAddElementOption($ddm_group, $element, 'blank_value')===true):?>
			<?php $data=array(''=>'')+$this->getAddElementOption($ddm_group, $element, 'data');?>
		<?php else:?>
			<?php $data=$this->getAddElementOption($ddm_group, $element, 'data');?>
		<?php endif?>
		<?php if(isset($data[$this->getAddElementStorage($ddm_group, $element)])):?>
			<div class="form-control readonly"><?php echo h()->_outputString($data[$this->getAddElementStorage($ddm_group, $element)])?></div>
		<?php else:?>
			<div class="form-control readonly">&nbsp;</div>
		<?php endif?>
	<?php else:?>
		<?php if($this->getAddElementOption($ddm_group, $element, 'blank_value')===true):?>
			<?php echo osW_Form::getInstance()->drawSelectField($element, array(''=>'')+$this->getAddElementOption($ddm_group, $element, 'data'), $this->getAddElementStorage($ddm_group, $element), array('input_class'=>'form-control selectpicker'))?>
		<?php else:?>
			<?php echo osW_Form::getInstance()->drawSelectField($element, $this->getAddElementOption($ddm_group, $element, 'data'), $this->getAddElementStorage($ddm_group, $element), array('input_class'=>'form-control selectpicker'))?>
		<?php endif?>
	<?php endif?>
