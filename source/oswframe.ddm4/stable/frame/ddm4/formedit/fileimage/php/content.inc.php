...
if ($this->getEditElementOption($ddm_group, $element, 'edit_enabled')) {
	osW_JQuery::getInstance()->loadPlugin('fancybox2');
...
');
	...
	osW_Session::getInstance()->set($ddm_group.'_'.$element, array('element'=>$this->getEditElement($ddm_group, $element), 'storage'=>$this->getEditElementStorage($ddm_group, $element), 'ddm_group'=>$ddm_group, 'ddm_element'=>$element, 'layout'=>$this->getGroupOption($ddm_group, 'layout')));
}
