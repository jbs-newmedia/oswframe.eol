...
<div class="form-group<?php if(osW_Form::getInstance()->getFormError($element)):?> has-error<?php endif?> ddm_element_<?php echo $this->getEditElementValue($ddm_group, $element, 'id')?>">
	<label class="control-label"><?php echo h()->outputString($this->getEditElementValue($ddm_group, $element, 'title'))?><?php if($this->getEditElementOption($ddm_group, $element, 'required')===true):?><?php echo $this->getGroupMessage($ddm_group, 'form_title_required_icon')?><?php endif?><?php echo $this->getGroupMessage($ddm_group, 'form_title_closer')?></label><br/>
		<?php if($this->getEditElementOption($ddm_group, $element, 'read_only')===true):?>
			...
			<?php if($this->getEditElementStorage($ddm_group, $element)==1):?>
		...
		<?php else:?>
			<?php if($this->getEditElementOption($ddm_group, $element, 'displayorder')=='yn'):?>
				<label class="radio-inline"><?php echo osW_Form::getInstance()->drawRadioField($element, '1', $this->getEditElementStorage($ddm_group, $element), array())?><?php echo h()->_outputString($values['options']['text_yes'])?></label>
				<label class="radio-inline"><?php echo osW_Form::getInstance()->drawRadioField($element, '0', $this->getEditElementStorage($ddm_group, $element), array())?><?php echo h()->_outputString($values['options']['text_no'])?></label>
			<?php else:?>
				<label class="radio-inline"><?php echo osW_Form::getInstance()->drawRadioField($element, '0', $this->getEditElementStorage($ddm_group, $element), array())?><?php echo h()->_outputString($values['options']['text_no'])?></label>
				<label class="radio-inline"><?php echo osW_Form::getInstance()->drawRadioField($element, '1', $this->getEditElementStorage($ddm_group, $element), array())?><?php echo h()->_outputString($values['options']['text_yes'])?></label>
			<?php endif?>
