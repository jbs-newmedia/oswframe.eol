...
foreach (osW_DDM4::getInstance()->getEditElements($ddm_group) as $element => $options) {
...
foreach ($ajax['logic'] as $group => $group_data) {
	foreach ($group_data['rule'] as $rule_key => $rule_value) {
		$this->parseElementPHP($ddm_group, 'datatableajax', $this->getEditElementValue($ddm_group, $rule_key, 'module'), array('module'=>$this->getEditElementValue($ddm_group, $rule_key, 'module'), 'rule_key'=>$rule_key, 'rule_value'=>$rule_value));
	...
	foreach ($ajax['logic'][$group]['view'] as $element) {
		...
		if (isset($ajax['logic'][$group]['reset'])) {
			foreach ($ajax['logic'][$group]['reset'] as $reset_key) {
				$this->parseElementPHP($ddm_group, 'datatableajax', $this->getEditElementValue($ddm_group, $reset_key, 'module'), array('module'=>$this->getEditElementValue($ddm_group, $reset_key, 'module'), 'reset_key'=>$reset_key));
		...
		if (isset($ajax['logic'][$group]['set'])) {
			foreach ($ajax['logic'][$group]['set'] as $set_key => $set_value) {
				$this->parseElementPHP($ddm_group, 'datatableajax', $this->getEditElementValue($ddm_group, $set_key, 'module'), array('module'=>$this->getEditElementValue($ddm_group, $set_key, 'module'), 'set_key'=>$set_key, 'set_value'=>$set_value));
